...
class _StudioHomeScreenState extends State<StudioHomeScreen> {
  ...
  Widget build(BuildContext context) {
    final screens = [
      ...
      const Center(child: Text("Seats")),    // Placeholder
...
class _StudioDashboard extends StatefulWidget {
  @override
  State<_StudioDashboard> createState() => _StudioDashboardState();
}

class _StudioDashboardState extends State<_StudioDashboard> {
  ...
  void initState() {
    super.initState();
    _repo = StudioRepository(ApiClient());
    _studiosFuture = _repo.getMyStudios().then((studios) async {
      if (studios.isNotEmpty) {
        // Fetch stats for the first studio
        final studioId = studios.first['id'];
        final stats = await _repo.getStudioStats(studioId);
